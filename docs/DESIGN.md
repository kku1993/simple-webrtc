...
3. **State loss is a normal event, not an error.** A crash, a redeploy, or a
   ...
   which let a client rebuild a room the server has forgotten.
...
- **Peer epoch** — a random string generated by a client each time it constructs
  a new `SimplePeer` instance. It tells the server whether a reconnecting client
  is continuing its existing WebRTC session or has started a fresh one. See
  [Peer epochs and signal buffering](#peer-epochs-and-signal-buffering).
...
| `peer.on('close')` / `peer.on('error')` | Reconnect via `rejoin-room`, new peer epoch |
...
connection (close `4400`) — see [Rejoining](#rejoinroom).
...
```typescript
{
  type: "create-room";
  requestId?: string;
  peerEpoch: string;                  // >=16 random bytes, base64url
...
```typescript
{
  ...
  requestId?: string;
  roomId: string;                 // server-assigned, 128 bits of entropy, base64url (22 chars)
  role: "host";
  rejoinToken: string;
  guestDeadlineAt: string;        // ISO; room is closed if no guest joins by then
  guestDeadlineInSeconds: number; // default 600
...
The `roomId` is generated by the server from a CSPRNG. Server assignment (rather
than client-supplied IDs) removes room-squatting entirely: no client can occupy
or pre-register an ID it was not given.

The host should display `guestDeadlineInSeconds` to the user and must **not**
...
```typescript
{
  ...
  peerEpoch: string | null;      // the host's current epoch, if any
...
- Joining clears the room's guest deadline. Only `roomExpiresAt` applies
  afterwards.
- A join link is frequently shared before the host has finished creating the
  room. Guests receiving `ROOM_NOT_FOUND` should retry with backoff for up to
  30 s before surfacing an error. This is a normal race, not a failure.
...
separately; the token is the sole authority on both.
...
```typescript
{
  ...
  peerEpoch: string | null;  // the other slot's current epoch, if any
  roomExpiresAt: string;
  roomExpiresInSeconds: number;
  rejoinTokenExpiresAt: string;
}
...
client must destroy its `SimplePeer`, construct a new one with a **new peer
epoch**, and wait (host) or begin signaling (guest) as if starting fresh.
...
4. Look up the room in memory:
   - **Found** → attach to `token.role`. If that slot already holds a
     ...
     out of its own room.
   - **Not found** → recreate the room with `createdAt` taken from the token,
     the guest password hash taken from the token, and a fresh in-memory
     lifetime. Attach to `token.role`. Respond with `recreated: true`.
5. Replay buffered signals if the peer epoch matches (see below).
...
```typescript
{
  type: "signal";
  requestId?: string;
  seq: number;    // starts at 1, increments per message, per (slot, peerEpoch)
...
|---|---|---|
| `guest-joined` | — | Host may now construct `SimplePeer({ initiator: true })` |
...
| `peer-rejoined` | `{ role, peerEpoch }` | Other side is back with the **same** epoch. Its WebRTC session continued |
| `peer-reset` | `{ role, peerEpoch }` | Other side is back with a **new** epoch. Destroy your `SimplePeer`, build a new one, roles unchanged |
...
| `room-expired` | — | In-memory lifetime reached. Followed by close `4014`. Rejoin is still permitted while the token is valid |
| `server-shutdown` | `{ reconnectAfterMs }` | Graceful drain. Followed by close `4300` |
| `error-response` | see below | Request failed |

## Peer epochs and signal buffering
...
which is what `peerEpoch` answers:

- **The WebSocket dropped but the page lived.** The client's
  `RTCPeerConnection` is still valid. It reconnects with the **same**
  `peerEpoch`. The server replays the buffer in order and sends
  `peer-rejoined` to the other slot.
- **The page reloaded, or the client destroyed and rebuilt its peer.** It
  reconnects with a **new** `peerEpoch`. The buffered signals belong to a dead
  session and replaying them would corrupt the new one. The server **discards**
  the buffer and sends `peer-reset` to the other slot, which must then destroy
  and rebuild its own `SimplePeer`. Roles do not change: the host remains the
  initiator.

Clients must persist `{ roomId, role, rejoinToken, peerEpoch, peerPeerEpoch }`
in `sessionStorage` so that a reload can distinguish "I am continuing" (rare)
from "I am fresh" (usual) — in practice a reload always means a new epoch, since
the `RTCPeerConnection` did not survive.
...
- The arriving client learns the other slot's epoch from `peerEpoch` in its
  `join-room-response` / `rejoin-room-response` (`null` if that slot has never
  been occupied in this room instance).
- The other slot is told the arriving epoch via `peer-rejoined` or `peer-reset`.

The server decides which of those two events to send by comparing the arriving
epoch to the epoch it currently has recorded for that slot. A recreated room has
no recorded epochs, so the *first* client to attach triggers no event (there is
nobody to notify) and the *second* one triggers `peer-reset`, since an unknown
prior epoch must be treated as changed.

The arriving client applies the same rule in reverse: if the `peerEpoch` it
receives differs from the `peerPeerEpoch` it had stored, it destroys and rebuilds
its `SimplePeer`. This symmetry is what makes recovery from a full server restart
work — both sides independently conclude they must start over, and neither waits
for the other.
...
Both the host's and the guest's token carry the same `roomId` and `createdAt`;
only `role` differs. The guest's token is issued on `join-room` and reuses the
room's original `createdAt`.

`serverSecret` is at least 32 random bytes, supplied via configuration. If it is
not set, the server generates one at startup and logs a warning that rejoin
tokens will not survive a restart — which defeats most of the point, so
...
The payload is deliberately sufficient to reconstruct a room exactly:
`roomId` identifies it, `role` says which slot the bearer takes, `createdAt`
bounds its lifetime, and the password hash preserves the room's access control
so a guest who lost its own token can still `join-room` with the password.

The token is a bearer credential. It is unguessable (128-bit `roomId` plus a
...
|---|---|---|---|
| Guest deadline | `guestDeadlineSec` | 600 (10 min) | How long a room waits for its first guest |
...
sequenceDiagram
    ...
    H->>S: ws /v1/signal
    H->>S: create-room {peerEpoch}
    S-->>H: create-room-response {roomId, rejoinToken, guestDeadlineAt}
    Note over H: shares roomId out of band

    G->>S: ws /v1/signal
    G->>S: join-room {roomId, peerEpoch, guestPassword?}
    S-->>G: join-room-response {rejoinToken}
    S-->>H: guest-joined
...
Recovery after the server lost all state:
...
sequenceDiagram
    ...
    H->>S: rejoin-room {rejoinToken, peerEpoch: NEW}
    Note over S: verify HMAC, check age, no tombstone,<br/>room not in memory -> recreate from token
    S-->>H: rejoin-room-response {recreated: true, peerConnected: false, peerEpoch: null}
    G->>S: rejoin-room {rejoinToken, peerEpoch: NEW}
    S-->>G: rejoin-room-response {recreated: false, peerConnected: true, peerEpoch: host's NEW}
    S-->>H: peer-reset {role: "guest", peerEpoch}
    Note over G: stored peerPeerEpoch != received peerEpoch -> rebuild
    Note over H: peer-reset -> rebuild
    Note over H,G: both rebuild SimplePeer, host still initiator
```
...
| `create-room` | per IP | 5/min, burst 10 | `RATE_LIMITED` |
...
- Compared with a constant-time comparison against
...
- `roomId` is validated for length and charset (22 base64url chars) before any
...
| 1101 | `ROOM_NOT_FOUND` | yes | Guest: backoff retry up to 30 s (link may precede creation), then surface. Host: create a new room |
...
| 4014 | Room expired (in-memory lifetime) | Rejoin with token if still valid |
...
on disconnect(closeCode, lastError):
  ...
  sleep(delay); attempt++
  reconnect and send rejoin-room with the stored token and a NEW peerEpoch
```
...
- Clients hold `{ roomId, role, rejoinToken, peerEpoch }` in `sessionStorage`.
...
- A pairing survives restarts for `rejoinTokenTtlSec` (12 h by default), so long
  as `serverSecret` is stable across restarts.
...
| `serverSecret` | `SERVER_SECRET` | *(generated)* | ≥32 bytes. **Must be set in production** |
...
| `guestDeadlineSec` | `GUEST_DEADLINE_SEC` | `600` | |
...
interface Room {
  ...
  guestDeadlineAt: number | null;  // null once a guest has joined
  guestPasswordHash: Uint8Array | null;
  guestPasswordSalt: Uint8Array | null;
  passwordAttempts: number;
  slots: Record<Role, Slot>;
  creatorIp: string;               // for the per-IP concurrent room limit
}
```
...
- One timer wheel (or a single sweep goroutine on a ~5 s tick) driving guest
...
1. WebSocket endpoint with origin check, global connection cap, and handshake
   timeout.
2. Token codec: sign, verify (constant-time), encode, decode, TTL check.
3. Room registry with the timer sweep for the three lifetimes.
4. The three handshake handlers, with the rejoin validation order given above
   implemented exactly.
5. Signal relay with per-slot ordered buffering, epoch comparison, duplicate
   suppression, and overflow handling.
6. Peer lifecycle events: `guest-joined`, `peer-disconnected`, `peer-rejoined`,
   `peer-reset`.
7. `peer-connected` tracking and the grace-period socket release.
8. Tombstone LRU and `close-room`.
9. Rate limiters and the guest-password attempt counter.
10. Turnstile verification on `create-room`.
11. Graceful shutdown with staggered `server-shutdown` notices.
12. `/healthz` and `/metrics`.
